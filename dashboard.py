...
def main() :
    ...
    def load_data():
        ...
        #données test avant feature engeniering
        X_test = pd.read_csv('dataset/X_test.parquet')
        
        #données train avant feature engeniering
        y_test = pd.read_csv('dataset/y_test.csv')        
        
        #description des features
        description = pd.read_csv('dataset/HomeCredit_columns_description.csv', 
                                      usecols=['Row', 'Description'], \
                                  index_col=0, encoding='unicode_escape')

        return df, data_test, data_train, y_test ,  description   # ,X_test 
...
########################################################################################################################################### 
    ...
    with st.sidebar:
        ...
        show_credit_decision = st.checkbox("Ddécision de crédit")
...
###########################################################################################################################################    
    ...
    ## display_client_info(str(client['SK_ID_CURR'].values[0]),str(client['AMT_INCOME_TOTAL'].values[0]),
     ##                str(round(client['DAYS_BIRTH'].values[0])),str(round(client['DAYS_EMPLOYED']/-365).values[0])) 
        ...
        default_list=\
        ["GENRE","AGE","STATUT FAMILIAL","NB ENFANTS","NB ANNEES EMPLOI","NIVEAU EDUCATION",
        ]
